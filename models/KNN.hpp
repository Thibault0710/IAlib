...
public :
    ...
    Matrix<int, Eigen::Dynamic, 1> feedforward(const Matrix<T, Eigen::Dynamic, Eigen::Dynamic> &Xtest) {// The data we want to label using KNN algorithm
    ...
    Matrix<int, Eigen::Dynamic, 1> feedforward(const std::vector<std::vector<T>> &Xtest) {
        return this->feedforward(vectorToEigenMatrix(Xtest));
    }
